...
{
	int ix, iz, ii;
	double invD, Uu, diff, tmp, obj=0.0;
	double *temp;
	invD = 1/4.0;
...
#pragma omp parallel for num_threads(size) \
	default(none) private(ix, iz, ii, Uu, tmp) \
	shared(N, u, temp, b)
	for(iz = 0; iz < N; iz++){
		for(ix = 0; ix < N; ix++){
			ii = ix+iz*N;
...
#pragma omp parallel for num_threads(size) \
	private(ix, iz, ii, Uu, tmp) \
	shared(N, u, temp, weight, invD)
	for(iz = 0; iz < N; iz++){
		for(ix = 0; ix < N; ix++){
			ii = ix+iz*N;
			Uu = 0.0;
			Uu -= (iz-1>=0)?u[ii-N]:0.0;
			Uu -= (ix-1>=0)?u[ii-1]:0.0;
			tmp = temp[ii] - Uu;
			//u[ii] = invD*tmp;
			u[ii] = weight*invD*tmp + (1.0-weight)*u[ii];
		}
	}
